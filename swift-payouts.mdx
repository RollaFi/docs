...
[`transaction.uetr.assigned`](/api-reference/webhooks/events#payout-tracking-events)
webhook — and once a payout has one, it is also returned on that payout by
[Get Transaction](/api-reference/endpoint/wallet/get-transaction) and the transaction
...
- **The webhook is how you learn it without asking.** Subscribe if you show payment
  references to your users or handle their tracing requests; otherwise read `uetr` off the
  payout whenever you need it. Both carry the same value.
