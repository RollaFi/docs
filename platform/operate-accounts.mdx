...
<Note>
  There is no need to create per-customer API keys: one tenant key operates the whole book. Keep it server-side and [IP-allowlisted](/concepts/authentication#ip-allowlisting) — every key needs one.
</Note>
