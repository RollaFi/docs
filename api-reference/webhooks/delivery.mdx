...
Every delivery attempt is logged and visible from your [Rolla Dashboard](https://app-staging.rolla.xyz) under **API → Webhooks → ⋯ → Deliveries**. Each entry can be expanded to show the exact **payload sent**, the request's target URL, and the **response** (status + body). It records:
