...
```json
{
  ...
  "data": {
    ...
    "reference": "…",
    "type": "deposit | payout",
    "status": "pending | processing | negotiating | sent | completed | failed | rejected | cancelled | refunded",
...
| `data.status` | The transaction status this event represents — one of `pending`, `processing`, `negotiating`, `sent`, `completed`, `failed`, `rejected`, `cancelled`, `refunded` (matches the `event` type). See [Events](/api-reference/webhooks/events) |
