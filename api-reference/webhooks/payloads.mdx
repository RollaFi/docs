...
| `data.status` | The transaction status this event represents — one of `pending`, `processing`, `sent`, `completed`, `failed`, `rejected`, `refunded` (matches the `event` type). See [Events](/api-reference/webhooks/events). On [`transaction.uetr.assigned`](#sample-payout-uetr-assigned), which announces no status change, it reports the payout's status instead — see that section. |
...
| `data.fee_breakdown` | How `fee_amount` splits between the parties that earned it, as an array of `{ party, amount, transaction_id, reference }`. Present only when a white-label partner charged a margin on top of Rolla's fee; **omitted** otherwise. `fee_amount` is still the authoritative total — this only says where each part went. |
...
| `data.virtual_account` | The destination virtual account a **deposit** was paid into — present for **deposits**, **omitted** for payouts. Lets you reconcile the deposit to a specific account. See [the `virtual_account` object](#the-virtual_account-object-deposits) below; it is close to, but not identical with, the object on [`account.virtual_account.created`](/api-reference/webhooks/events). |
| `data.payer` | The sender/originator of a **deposit** — present for **deposits** when the provider supplies sender details, and on the receiving side of a [Rolla transfer](/api-reference/endpoint/wallet/transfer); **omitted** for payouts. See below for availability by rail. |
| `data.payment_rail` | Which rail an incoming **deposit** arrived on — `ach`, `wire_domestic`, `wire_international` or `mobile_money`. Present once the provider reports the transfer method, which in practice means **USD bank deposits** and mobile money collections, and on sandbox [simulated deposits](/api-reference/endpoint/accounts/simulate-deposit#choosing-the-rail); **omitted** otherwise, and never present on payouts. Not to be confused with `data.rail` above, which is `fiat` or `stablecoin`. See [Which rail a deposit arrived on](#which-rail-a-deposit-arrived-on). |
...
| `data.metadata` | Any metadata you supplied when creating the transaction — a [payout](/api-reference/endpoint/wallet/withdraw#attaching-your-own-metadata) or a [Rolla transfer](/api-reference/endpoint/wallet/transfer#attaching-your-own-metadata). Returned exactly as you sent it; **omitted** when you supplied none. On a transfer's receiving side it appears only when both accounts share an owner |
...
<Note>
  **Webhook amounts match the REST transaction endpoints.** Both report amounts as **major × 100** (e.g. `1000` = $10.00), so the same transaction shows `1000` in its webhook and `1000` over REST. Match a webhook to a REST record by **`transaction_id`** (or `reference`) rather than by comparing amounts — a reference is stable where an amount is not unique.

  Before **9 September 2026** the REST transaction responses reported major units (`10` = $10.00) while webhooks reported `1000`. If your integration converted between the two, remove that conversion.
</Note>
...
<Warning>
  This object is a snapshot for reading, not a request body. If you feed it back into [Create Beneficiary](/api-reference/endpoint/beneficiaries/create) or [Update Beneficiary](/api-reference/endpoint/beneficiaries/update), convert any `country` that is not already a 2-letter ISO code, and drop `id`. Those endpoints reject a full country name with a `400`.
</Warning>
...
| `method` | Fields |
|---|---|
| `bank_transfer` | `id` (the virtual account's Rolla id), `currency`, `bank_name`, `account_number`, `account_name`, plus `routing_number` / `swift_code` when the account has them, and `reference` / `label` when you set your own id and name on it |
| `crypto` | `id`, `currency`, `wallet_address`, `network` |
| `mobile_money` | `id`, `currency`, `provider` (the mobile money operator), `phone_number`, `account_name` — this is the customer's own wallet the money came from, not an account Rolla issued |

<Note>
  The object on a **deposit** carries no `provider` and no `status` (except the operator name on the
  `mobile_money` shape). The `virtual_account` on
  [`account.virtual_account.created`](#sample-virtual-account-created) is a different, slightly wider
  object: it does carry `provider` (always `rolla`) and `status`. Don't assume one from the other.
</Note>
...
| `mobile_money` | A mobile money collection (e.g. GHS wallet funding) |
...
<Info>
  There is no `swift` value: an international wire **is** the SWIFT case, reported as `wire_international`.

  `payment_rail` is **omitted** — not `null` — when the provider does not report a transfer method, and it never appears on payouts or crypto deposits. Treat its absence as "rail unknown", not as a specific rail.
</Info>

<Note>
  In sandbox, pass `payment_rail` (`ach`, `fedwire` or `swift`) to [Simulate an Account Deposit](/api-reference/endpoint/accounts/simulate-deposit#choosing-the-rail) or [Simulate a USD Deposit](/api-reference/endpoint/wallet/simulate-usd-deposit) to receive each value here. Simulated deposits default to `ach`.
</Note>
...
{
  "event": "transaction.completed",
  "event_id": "3d9a7c15-6e22-5b44-9f13-2c8e5a1d7b60",
  "created_at": "2026-06-10T12:00:05.000Z",
  "data": {
    "transaction_id": "5f1c9d0a-2b73-4e86-9a5c-3d8f1e6b0c42",
    "reference": "US-DEP-7K4M2B",
    "type": "deposit",
    "status": "completed",
    "rail": "fiat",
    "currency": "USD",
...
The receiving side of a [Rolla transfer](/api-reference/endpoint/wallet/transfer) — for example a merchant account forwarding funds to your institutional account. `payer` names the sending account, and `metadata` is included because both accounts share an owner.

```json
{
  "event": "transaction.completed",
  "event_id": "5e0c2a71-9b3d-5f4e-8a61-2c7d9e0f4b18",
  "created_at": "2026-09-17T14:37:19.976Z",
  "data": {
    "transaction_id": "c7a1e9d2-4b6f-4e83-9a05-7d2f1b8c3e64",
    "reference": "TRANSFER-1789655829756-4f2a9c1e-CREDIT",
    "type": "deposit",
    "status": "completed",
    "rail": "fiat",
    "currency": "USD",
    ...
    "destination_currency": "USD",
    "fee_amount": 0,
    "description": "Transfer from Northwind Trading Ltd",
    "timestamp": "2026-09-17T14:37:09.814Z",
    "payer": {
      "name": "Northwind Trading Ltd",
      "bank_name": "Rolla"
    },
    "metadata": { "original_payer": "Contoso Retail UK Ltd", "order_id": "ORD-88213" }
  }
}
```
...
available — see [Events](/api-reference/webhooks/events#payout-tracking-events). Its
...
| Field | Description |
|-------|-------------|
| `data.transaction_id` / `data.reference` | The payout the reference belongs to — match on either. |
| `data.type` | Always `payout`. |
| `data.status` | The payout's status **at the time the reference was recorded**. Unlike the status events, this one carries no status of its own, so `status` reports the row — commonly `sent`, and `completed` when the payout settled before the reference came back. It is context, **not** a status change: never advance your own state machine on it. |
| `data.uetr` | The UETR itself. |
| `data.tracking_codes` | Every tracking code recorded against the payout, keyed by name — the `UETR` always, and `IMAD` on the corridors that produce one. Read `data.uetr` for the UETR; use this object if you also surface the others. |
| `data.currency` / `data.amount` | The beneficiary's currency and amount, in **minor units** (see [Amount format](#amount-format)) — enough to confirm you matched the right payout. |
| `data.beneficiary` | The payout's beneficiary, in the [same shape](#the-beneficiary-object-payouts) as the status events. |
| `data.timestamp` | When the reference was **recorded** (ISO 8601). Earlier than `created_at`, which is when this event was generated — quote `timestamp` when tracing a payment. |
| `data.metadata` | Any metadata you supplied when [creating the payout](/api-reference/endpoint/wallet/withdraw#attaching-your-own-metadata), passed through unchanged. |

<Note>
  The fee, FX and amount-leg fields (`source_amount`, `exchange_rate`, `fee_amount`, …)
  are **not** on this payload — nothing about the money changed. Take them from the
  payout's own status events.
</Note>
...
Payer name events (`payer_name.approved`, `payer_name.changes_requested`, `payer_name.rejected`, `payer_name.active`, `payer_name.action_required`) describe a **payer name** — the name a beneficiary sees on payouts you send under [Named Payouts](/guides/named-payouts). `data` carries the payer name; `transaction_id` is not present.

| Field | Type | Description |
|-------|------|-------------|
| `data.payer_name_id` | string | The payer name the event is about. This is the id you pass as `payerNameId` on a payout. |
| `data.name` | string | The name the beneficiary sees. |
| `data.type` | string | `business` or `individual`. |
| `data.country` | string | ISO 3166-1 alpha-2 country code of the payer. |
| `data.status` | string | Where the payer name has got to: `pending_review`, `changes_requested`, `processing`, `active`, `action_required`, `rejected` or `archived`. |
| `data.is_default` | boolean | Whether this name is applied to payouts that do not name one. |
| `data.reason` | string \| null | The reviewer's note, verbatim. Set on `payer_name.changes_requested` and `payer_name.rejected`, `null` otherwise. Safe to show your customer. |
| `data.metadata` | object | Any metadata you supplied when registering the name, passed through unchanged. Omitted if you supplied none. |
| `data.created_at` | string | When the payer name was submitted (ISO 8601). |
| `data.updated_at` | string | When it last changed (ISO 8601). |
...
```json
{
  "event": "payer_name.active",
  "event_id": "7c8d9e0f-1a2b-5c3d-8e4f-5a6b7c8d9e0f",
  "created_at": "2026-09-02T11:02:41.000Z",
  "data": {
    "payer_name_id": "9f8e7d6c-5b4a-4938-8271-0a1b2c3d4e5f",
    "name": "Acme Trading Ltd",
    "type": "business",
    "country": "NG",
    "status": "active",
    "is_default": true,
    "reason": null,
    "created_at": "2026-09-01T09:14:22.000Z",
    "updated_at": "2026-09-02T11:02:41.000Z",
...
```json
{
  "event": "payer_name.changes_requested",
  "event_id": "1a2b3c4d-5e6f-5071-8293-a4b5c6d7e8f9",
  "created_at": "2026-09-01T16:45:12.000Z",
  "data": {
    "payer_name_id": "9f8e7d6c-5b4a-4938-8271-0a1b2c3d4e5f",
    "name": "Acme Trading Ltd",
    "type": "business",
    ...
    "status": "changes_requested",
    "is_default": false,
    "reason": "The registration number does not match the certificate you attached. Correct it and resubmit.",
    "created_at": "2026-09-01T09:14:22.000Z",
    "updated_at": "2026-09-01T16:45:12.000Z"
  }
}
```

Answer it with [Update Payer Name](/api-reference/endpoint/payer-names/update), which resubmits the name for review.
