...
| `data.payment_rail` | Which rail an incoming **deposit** arrived on — `ach`, `wire_domestic` or `wire_international`. Present on **USD bank deposits** once the provider reports the transfer method; **omitted** otherwise, and never present on payouts. Not to be confused with `data.rail` above, which is `fiat` or `stablecoin`. See [Which rail a deposit arrived on](#which-rail-a-deposit-arrived-on). |
...
| `data.metadata` | Any metadata you supplied when creating the transaction — see [Attaching your own metadata](/api-reference/endpoint/wallet/withdraw#attaching-your-own-metadata). Returned exactly as you sent it; **omitted** when you supplied none |
...
<Note>
  Deposits created with the sandbox simulate endpoint do not currently carry a `payment_rail`, so you will only see this field on deposits received through the bank rails.
</Note>
