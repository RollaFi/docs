...
- **USD** — `name`, `account_number`, `bank_name` and `country` as the sending rail reports them.
...
`payment_rail` tells you how an incoming USD payment reached the account, so you can reconcile per channel instead of parsing the narration.
