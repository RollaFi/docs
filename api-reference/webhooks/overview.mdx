...
Webhooks let Rolla notify **your** server in real time whenever one of your deposits or payouts changes status — so you no longer need to poll the transaction endpoints. They also carry the things polling could never tell you, such as a payout's [UETR](/api-reference/webhooks/events#payout-tracking-events), which the transaction endpoints do not return. When an event occurs, Rolla sends an HTTP `POST` request to a URL you configure, with a JSON payload describing the transaction.
