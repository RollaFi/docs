...
Webhooks let Rolla notify **your** server in real time whenever one of your deposits or payouts changes status — so you no longer need to poll the transaction endpoints. They also announce things that appear well after a payout is sent, such as its [UETR](/api-reference/webhooks/events#payout-tracking-events) — which you can read back from the transaction endpoints, but only if you know to go looking. When an event occurs, Rolla sends an HTTP `POST` request to a URL you configure, with a JSON payload describing the transaction.
