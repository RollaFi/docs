...
- Stablecoin deposits and payouts (USDC, USDT) across supported chains
...
| **Signing secret** | Generated for you on creation. Used to [verify signatures](/api-reference/webhooks/signatures) |

You can register multiple endpoints (for example, separate URLs per environment), enable or disable each one, and rotate its signing secret at any time.

<Info>
  Your signing secret can be viewed any time from the dashboard (**API → Webhooks → ⋯ → Reveal secret**), so you don't need to store it elsewhere. If you suspect it's compromised, rotate it with **Regenerate secret** — the old secret stops working immediately.
</Info>
