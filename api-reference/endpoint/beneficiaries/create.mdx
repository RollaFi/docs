...
<Info>
  `routing_number` is the beneficiary bank's 9-digit ABA number and is **required** for `ach` — an ACH credit is addressed by routing number, so a beneficiary saved without one is rejected with a `400`.
</Info>
