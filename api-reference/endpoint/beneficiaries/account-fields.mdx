...
|-------|-------------|---------------|
| `account_owner_type` | `individual`, `business` | Bank-account beneficiaries in currencies other than NGN — `ach`, `domestic_wire`, `international_wire`. **Not applicable** to `crypto_usdt`, `crypto_usdc`, `rolla_transfer`, `mobile_money`, or NGN beneficiaries (NGN payouts settle locally and carry no individual-vs-business routing meaning). |
...
<Tip>
  Crypto (`crypto_usdt`, `crypto_usdc`), `rolla_transfer`, `mobile_money`, and **NGN** beneficiaries
  do **not** need `account_owner_type` — they're exempt from routing classification. `account_category`
  still applies whenever `currency` is `USD`, regardless of rail.
</Tip>

## Changelog

<Update label="NGN exempted from account_owner_type">
  NGN bank-account beneficiaries no longer need `account_owner_type` — NGN payouts settle
  locally and carry no individual-vs-business routing meaning. `account_category` is
  unaffected; it still applies whenever `currency` is `USD`.
</Update>
