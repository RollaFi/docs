...
<Warning>
  An email on Rolla's blocklist is refused with `403` and `code: "EMAIL_BLOCKLISTED"`:

  ```json
  {
    "status": 403,
    "message": "This email address cannot be used. Please contact Support for assistance.",
    "code": "EMAIL_BLOCKLISTED"
  }
  ```

  This is separate from the white-label `403` above, which has no `code`. Branch on `code`, not on the status alone.
</Warning>
