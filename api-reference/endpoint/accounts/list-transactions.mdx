...
<Info>
  ...
  {
    "status": 403,
    "message": "This endpoint lists transactions across the accounts you manage, and is available to partner API keys only."
  }
  ```
</Info>
    ...
    "transactions": [
      {
        ...
        "updated_at": "2026-08-19T10:00:02.000Z"
      },
      {
        ...
        "updated_at": "2026-08-19T10:00:01.000Z"
      }
    ],
    "totalCount": 4182,
    "page": 1,
...
Every row carries the same fields as the per-account listings, plus `account_id` and
`account_name`. Fields that do not apply are **omitted** rather than returned as `null`.
...
The file has one header row and these columns: `transaction_id`, `account_id`, `account_name`,
`transaction_type`, `status`, `source_amount`, `source_currency`, `destination_amount`,
`destination_currency`, `fee_amount`, `external_reference`, `fee_reference`,
`related_reference`, `created_at`, `updated_at`. It is sent as an attachment named
`transactions.csv`.

Paging comes back in the `X-Total-Count` and `X-Total-Pages` response headers, since a CSV body has nowhere to carry it. `page` and `pageSize` still apply, so a full export means walking the pages.
