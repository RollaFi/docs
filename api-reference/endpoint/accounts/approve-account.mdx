...
{
  "status": 409,
  "message": "Account must be submitted before it can be approved (current status: draft). Complete onboarding and submit the application first.",
  "code": "ACCOUNT_NOT_SUBMITTED"
}
```

The submission gate is re-run at approval time. If the account would no longer pass it, the call returns `400` whose `message` names the blocking requirement, the same message [Submit Application](/api-reference/endpoint/accounts/submit) reports.

## Example Request

```bash
curl -X POST "https://api-staging.rolla.xyz/api/v1/external/accounts/eec3cbed-79d8-4370-87a0-b6be9e287337/approve" \
