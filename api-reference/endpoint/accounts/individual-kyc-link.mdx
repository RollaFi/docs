...
```json
{
  ...
  "data": {
    ...
    "status": "active",
    "expiresAt": "2026-09-20T23:32:12.902Z",
...
<Tip>
  The link `status` becomes `completed` once the account holder passes verification, or `rejected` if it fails.

  After a failure, generate a fresh link and the previous failed attempt is cleared for you, so the account holder can try again. You do not need to contact Rolla. A link already marked `rejected` never becomes usable again — always send the customer the new one. An account that has already **passed** is left untouched: generating a link never discards a good verification.
</Tip>
