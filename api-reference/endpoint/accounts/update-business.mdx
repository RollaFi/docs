...
|---------|-----------------|
| `business_info` | `legalName`, `businessType`, `incorporationDate`, `incorporationCountry`, `registrationNumber`, `taxId`, `description`, `monthlyVolume`, `annualRevenue`, `primaryFundingSource`, `website` |
...
<Info>
  **`website`** is required because the USD issuing provider rejects an entity without one — a business account can't be submitted, let alone [issued a USD account](/api-reference/endpoint/accounts/issue-bank-account), without it.
</Info>
