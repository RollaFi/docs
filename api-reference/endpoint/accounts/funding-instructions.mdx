...
openapi: 'GET /accounts/{accountId}/funding-instructions'
---
...
|-----------|------|----------|-------------|
| `currency` | string | No | Scope to a single currency (e.g. `NGN`, `USD`, `USDC`), 2 to 10 characters. Omit to return every funded currency. |

`pending` is only populated when `currency` is omitted or `USD`; it is empty once the USD account is live.
