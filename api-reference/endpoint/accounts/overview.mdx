...
<Steps>
  <Step title="Create the account">
    `POST /accounts` with `type: "business"`, the business name and the account's own email. You get back the new account's `id` (use it as `accountId` below) and a draft application.
  </Step>
...
<Steps>
  <Step title="Create the account">
    `POST /accounts` with `type: "individual"` and the account's own email.
  </Step>
