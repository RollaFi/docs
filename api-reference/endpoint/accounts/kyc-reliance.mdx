...
<Warning>
  Individual accounts **only**. Calling this on a business account returns `400`. A business account's owners and directors must each complete verification through the [related-person KYC link](/api-reference/endpoint/accounts/kyc-link).
</Warning>
