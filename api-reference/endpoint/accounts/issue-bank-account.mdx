...
If the provider still needs details that are neither on the application nor in `accountData`, the call returns `400` with code `MISSING_ACCOUNT_DETAILS` listing them (see [Errors](#errors)).
...
A repeat call (with or without a `reference`) returns the existing account with `200` instead of `201`:
...
Error bodies carry `status` and `message`; the ones below also carry a `code`.
...
USD provider-side validation failures return `400` with an `errors` array; a duplicate NGN issuance returns `409` with a plain `message`; malformed request bodies return `400` with `"message": "Validation failed"` and an `errors` array.
