...
```json
{
  ...
  "data": {
    "application": {
      ...
      "updated_at": "2026-06-13T09:02:44.918Z"
    },
    "documents": []
  }
}
...
<Info>
  **Countries are normalized to [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2).** `citizenship` and `countryOfResidence` accept a code (`NG`), a country name (`Nigeria`) or a demonym (`Nigerian`), and are stored as the code. A value that can't be resolved is rejected with `400`. `address.country` is stored exactly as you send it, so send the ISO-2 code there.
</Info>
