...
<Warning>
  Identity verification **blocks submission**. The applicant must complete their [KYC link](/api-reference/endpoint/accounts/individual-kyc-link) before [Submit Application](/api-reference/endpoint/accounts/submit) will succeed — until then it returns `409 KYC_INCOMPLETE` and `readyToSubmit` stays `false`.
</Warning>
