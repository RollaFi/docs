...
Members are returned newest first. `invite_status` is `pending` until the member accepts the invitation, then `accepted` (or `declined` if they refuse). `role` and `role_id` are `null` for a member who has no role assigned.
