...
The roles returned depend on the caller's own role. Business Owners and Business Admins receive the full assignable set (`approver`, `business_admin`, `initiator`); other roles only see `initiator`. The `business_owner` and `individual_owner` roles are reserved and never returned by this endpoint. Roles come back sorted by `name`.
