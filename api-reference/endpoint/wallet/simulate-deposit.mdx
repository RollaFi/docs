...
openapi: 'POST /wallet/virtual-accounts/simulate-deposit'
---
...
| `provider` | string | No | Sandbox only. Overrides which payment rail the simulated deposit is tagged against: one of `HABARI`, `NOMBA`, `VFD`, `NETMFB`. Omit to inherit the target account's own rail — recommended, and the closest match to a real deposit. |
...
`customer_identifier` is echoed only when you targeted the account that way; a request by `account_number` omits it. `status` is the transaction's own status, `completed` once the deposit has been posted.

An account number or customer identifier that does not match an **active** virtual account of yours returns `404`, rather than crediting anything:

```json
{
  "status": 404,
  "message": "No active virtual account found for account_number \"9999999999\" under this business"
}
```

A body that fails validation, such as neither or both targeting fields or a non-numeric `amount`, returns `400` with `"message": "Validation failed"` and an `errors` array.
...
<Note>
  `amount` must be **greater than the pay-in fee** on the account. A smaller amount is rejected with `400` and the message `Deposit amount is too small — it must be greater than the payin fee configured on this account. Try a larger amount.`
</Note>
