...
|-----------|------|----------|-------------|
| `currency` | string | Yes | Code of the wallet being debited (3 or 4 letters, e.g. `NGN`, `USDC`) |
| `transactionType` | string | Yes | `deposit`, `withdrawal`, `swap`, `card_funding`, or `payin` |
| `amount` | number | Yes | Whole number of the smallest unit (kobo for NGN, cents for USD) |
...
| `rolla_transfer` | Transfer to another Rolla account |
...
`destination_country` still wins over the beneficiary's own value when you send it. `withdrawal_method` does not: a withdrawal always prices off the beneficiary's saved rail, so sending a `withdrawal_method` that contradicts it returns a `400` explaining which rail the payout will actually be charged as. Omit it, or update the beneficiary's `withdrawal_method` first.
