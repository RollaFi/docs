...
<Info>
This endpoint is **idempotent per `customer_identifier`**. Re-requesting for the same customer returns the account already on file rather than creating a duplicate: HTTP `200` with `"message": "Customer virtual account already exists for this customer_identifier"` and `existing: true`, against HTTP `201` and `existing: false` when a new account is minted. Use a **new** `customer_identifier` to create a **new** account for a different user.
</Info>
