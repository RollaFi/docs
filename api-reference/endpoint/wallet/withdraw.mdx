...
By default a payout reaches the beneficiary under Rolla's own payer details. If
[Named Payouts](/guides/named-payouts) is enabled for your account, pass `payerNameId` to send
it under one of your own registered payer names instead — your company, or the customer you are
paying on behalf of.
...
Omit `payerNameId` and the payout uses your [default payer name](/api-reference/endpoint/payer-names/set-default) if you have set one, and Rolla's standard payer details if you have not.
...
<Info>
  Payer names must be registered and reviewed before they can be used — you cannot pass a free-text
  payer name on a payout. Register them with [Create Payer Name](/api-reference/endpoint/payer-names/create).
</Info>
