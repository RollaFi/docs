...
| `metadata` | object | No | Your own key/value data, stored on the transaction and returned on every response and webhook for it. See [Attaching your own metadata](#attaching-your-own-metadata) |
| `payerNameId` | string (UUID) | No | Which of your registered payer names the beneficiary should see on this payout. Requires [Named Payouts](/guides/named-payouts) to be enabled. See [Choosing the payer name](#choosing-the-payer-name) |
...
A payout reaches the beneficiary under your own business name by default. If
[Named Payouts](/guides/named-payouts) is enabled for your account, pass `payerNameId` to send it
...
Omit `payerNameId` and the payout uses your [default payer name](/api-reference/endpoint/payer-names/set-default) if you have set one, and your own business name if you have not.

The id must be a payer name on your own account, and it must not be rejected or archived —
either is refused with a `400` and no payout is created. A name that has not finished
...
[Named Payouts guide](/guides/named-payouts) covers the statuses and the webhook that tells you
when a name is ready.

<Info>
  Affiliated payer names must be registered and reviewed before they can be used — you cannot pass a
  free-text payer name on a payout. Register them with [Create Payer Name](/api-reference/endpoint/payer-names/create).
</Info>
...
Pass a `metadata` object to carry your own identifiers on the payout — an order id, a
...
**Limits.** The object is stored on the transaction and replayed on every webhook
delivery for it, so it is bounded:
...
Exceeding any of these returns `400` and the payout is not created.

<Warning>
  Metadata is returned to anyone who can read the transaction and is sent to your webhook
  endpoints. Don't put secrets, card numbers, or personal data in it that you wouldn't
  want echoed back.
</Warning>

<Info>
  On `multipart/form-data` requests (USD international wire), send `metadata` as a
  **single JSON-stringified form field**, the same way `inlineBeneficiary` is sent.
</Info>
