...
The account must already have an **issued, active USD account**. Issue one first via [Issue a Bank Account](/api-reference/endpoint/accounts/issue-bank-account) with `currency: "USD"`. If no USD account is provisioned yet, the call returns `404`:
...
| `sender_name` | string | No | Name to record as the sender/counterparty on the transaction, returned as `payer.name`. Defaults to `"Sandbox Sender"` when omitted. |
| `payment_rail` | string | No | Rail the deposit should arrive on: `ach` (default), `fedwire` or `swift`. Reported on the deposit as `ach`, `wire_domestic` or `wire_international` — see [Choosing the rail](/api-reference/endpoint/accounts/simulate-deposit#choosing-the-rail). |
...
`reference` is the one you sent, or the generated `RL_SIM_...` when you omitted it. `bankAccountReference` names which deposit account was credited and is `null` for the primary account, which is the only one this endpoint targets. `bankAccountReference` and `transferId` are only present when the USD provider's sandbox reports them. A malformed body (for example a non-numeric `amount`) returns `400` with `"message": "Validation failed"` and an `errors` array; a rejection from the provider sandbox returns `400` with its message.
...
3. The transaction is completed, emitting the deposit webhook to your configured endpoint: a `transaction.pending` event, followed by `transaction.completed`. Both carry `payer.name` (your `sender_name`, or `"Sandbox Sender"`) and `payment_rail` for the rail you chose. See [Webhook Payloads](/api-reference/webhooks/payloads).

<Note>
  The rail's pay-in fee is taken from the deposit before it is credited, so simulate an amount larger than that fee. If Rolla cannot record the simulation's sender and rail, the request fails and no transfer is submitted, so it is safe to retry.
</Note>
