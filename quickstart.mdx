...
- A Rolla business account at [app.rolla.xyz](https://app-staging.rolla.xyz)
...
<Steps>
  <Step title="Log in to your dashboard">
    Go to [app.rolla.xyz](https://app-staging.rolla.xyz) and sign in.
  </Step>
...
```bash
curl -X POST "https://api-staging.rolla.xyz/api/v1/external/lookup" \
  -H "X-API-Key: your_api_key_here" \
...
```bash
curl -X POST "https://api-staging.rolla.xyz/api/v1/external/beneficiaries" \
  -H "X-API-Key: your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{
    "currency": "NGN",
...
```bash
curl -X POST "https://api-staging.rolla.xyz/api/v1/external/wallet/withdraw" \
  -H "X-API-Key: your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{
    "amount": 100000,
